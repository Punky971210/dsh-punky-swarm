...
- **工具 6 属性（P7）**：每工具提供 toolId / name / description / version / inputParam / outputParam（toolId = `dsh.punky-swarm.<name>` 反向域唯一；inputParam/outputParam 为 JSON Schema，required 恒在）；
...
- **身份体系（P2/P3，默认关）**：AIC 身份码（前缀 1.2.156.3088 + 10 级编码 + CRC-16/CCITT-FALSE + Base36 校验码）+ CAI 身份证书（CN=AIC、SAN=acps://、EAB 凭证）+ 可插拔签名（默认 ECDSA-P256 / 可选 RSA-2048）+ 信任链验证；SM2 暂不支持（`algorithm='sm2'` 显式拒绝）；装配键 `aip.identity` 默认关（经模块 API 暴露，不注册新治理工具）；
