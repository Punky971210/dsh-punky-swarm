...
> 本文档描述工具调用级护栏：6 原语运行期语义、规则配置与示例、拒绝收据与验签、事件桥接、热更新、不提供项及能力边界与取舍。批级治理引擎见 [governance-technical.md](governance-technical.md)；能力边界声明见 [governance-boundaries.md](governance-boundaries.md)。
...
相对任务级治理，护栏带来的治理能力增强共 10 项：

1. **治理粒度细化：任务级（批级编排内、派发前）→ 调用级（每次调用执行时）** —— 越界判定不再止于派发前，而是发生在每次工具调用上（§1）；
2. **双层独立防线** —— 任务级难度门禁（任务怎么派）与调用级护栏（每次调用是否越界）独立接线、串行叠加，互相不可绕过（文首对照表）；
3. **6 原语逐调用** —— ALLOW / DENY / REQUIRE_APPROVAL / NARROW / DEFER / PAUSE 在每次调用上落地，非 ALLOW 同步产出拒绝收据（§2）；
4. **确定性纯函数** —— 裁决内核同步、确定性、零 IO、无外部状态：同输入同输出，可单测、可复现（§2）；
5. **可验证的审计哈希链** —— 同 session 收据按 ts 序串 sha256 链，篡改即断链、verify 可定位（§4）；
6. **运行时状态 DEFER/PAUSE** —— 会话级延后/暂停以文件态真实生效（窗口 + 惰性过期恢复），非仅提示（§2）；
7. **热更新** —— 规则/开关/默认值运行期即时生效，免重启（§6）；
8. **双层联动违规升级** —— 调用级违规计数达阈值，经棘轮校验可触发批级 paused（§5）；
9. **出厂安全默认** —— 护栏默认启用、兜底 fail-closed（未分类违规 → DENY，绝不 ALLOW）：装上即有防线；出厂零拦截、需配规则才实际拦截，与之互补（见 8.2-2）；
10. **统一拒绝格式** —— `[governance:<primitive>]` 前缀对齐难度门禁风格，调用方可区分「任务级未评估」与「调用级越界」（§2）。

### 8.2 能力边界（8 项）

护栏能力的使用边界（范围声明，非缺陷；功能级不提供项清单见 §7）：

1. **工具侧、无模型侧** —— 裁决作用于宿主工具调用层（pre/post-execute），模型自身的生成与推理不在护栏范围内（见 §7）；
2. **出厂零拦截，需配规则才生效** —— `rules: []` 时 decide 恒 ALLOW；「装上即用、不改变既有行为」以配置规则为生效前提（§3）；
3. **单机、插件级，不做网关** —— 进程内、单机、插件形态；进程外网关 / MCP 网关路径不提供（见 §7）；
4. **NARROW 指引级、宿主禁改写** —— NARROW 以 deny + 收窄指引落地，不做透明参数改写（宿主禁止输入改写，§2/§7）；
5. **哈希链防篡改 ≠ 防删除** —— 链保证已落盘收据内容不可静默篡改（篡改即断链可定位），但收据文件本身可被删除：不提供 WORM / write-once（§4/§7）；
6. **归属依赖登记** —— 收据→批级联动的归属依赖 member.dispatch 会话映射登记；未归属的收据只留事件可见性、不触发批级状态迁移（§5）；
7. **无权威签名** —— 哈希锚定链内自证，无第三方权威签名背书（完整 RFC8785 与真签名不提供，§4/§7）；
8. **规则版本未快照** —— 收据记录命中规则 ID（ruleRefs）与理由，不记录裁决时刻的规则集版本；热更整体替换规则后，旧收据无法精确复现当时裁决所用的规则集（§4/§6）。

### 8.3 技术取舍

**优点**
- **确定性可测** —— 纯函数同步内核、同输入同输出，规则行为可单测锁定、可复现复核；
- **零新增依赖** —— 仅 Node 标准库 + 宿主 peer，无第三方运行时依赖（§2）；
- **零宿主改造** —— 插件侧增量接线，宿主只暴露既有生命周期事件，无侵入（§1）。

**代价**
- **无模型侧防线** —— 模型自身行为不在防线内（见 8.2-1）；
- **NARROW 不实改** —— 只指引收窄、不代改参数，实际效果受调用方配合约束（见 8.2-4）；
- **无 WORM** —— 防篡改不防删除，留痕强度上限为链式自证（见 8.2-5）；
- **在途审批热更重置** —— 热更重挂后，跨重挂在途 ask 的 outcome 补记丢失（`ask.initiated` 已于 pre 落盘，审计不丢；§6）。
