...
function effectiveRole(t: WaveTask): string | null {
...
export function collectRoleCompletenessWarnings(tasks: WaveTask[], waves: string[][]) {
  ...
  if (auditLanes.length > 0 && !auditLanes.some((t) => {
    const r = effectiveRole(t);
    return r !== null && AUDIT_LEAD_ROLES.has(r);
  })) {
    warnings.push({
      code: 'GATE_ROLE_MISSING',
      layer: 'audit',
...
export const MANAGER_PLANS = ['raise', 'leader-direct'] as const; // assembly.managerPlan 合法枚举（运行时事实源，与 contracts.ManagerPlan 字面量等价）
...
export function normalizeAssemblyDecl(input: unknown): { decl: WavePlanAssemblyDecl | null; warnings: WavePlanDoc['warnings'] } {
...
export function assemblyGate(tasks: WaveTask[], decl: WavePlanAssemblyDecl | null): AssemblyGateResult {
  ...
  if (!isCPlusBatch(tasks)) return 'ok';
  if (decl == null) {
    return {
...
export function normalizeTargetsContract(t: WavePlanTaskInput): { targets: string[] | null; targetsMarker: string | null } {
...
function validateLayerContract(tasks: WaveTask[]) {
...
export function validateWavePlan(plan: WavePlanDoc): boolean {
