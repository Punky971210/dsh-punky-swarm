...
// 全部命中行为空/禁用 → declared=false（零感知）
export function detectGate(artifactsDir, paths) {
    if (!Array.isArray(paths) || paths.length === 0)
        return { declared: false, commands: [], path: null };
    const commands = [];
...
export function createGates(root) {
    ...
    // ---- Tier3 门禁辅助 ----
    // lane 启动时间（targets 门禁基准）：events 末尾向前反查最近一条 `member.settled` 且 lane 匹配且
    ...
    function laneStartedAt(batch, lane) {
        ...
        for (let i = evs.length - 1; i >= 0; i--) {
            const e = evs[i];
            if (e && e.type === EVT.EVT_MEMBER_SETTLED && e.lane === lane && e.to === 'running' && e.ts)
                return e.ts;
        }
    ...
    // Entry Gate：exec 与 audit lane 派发前 consume 必须全部存在且非空
    // 条件覆盖 audit 层（原仅 exec——audit lane 声明 consume 时缺产物同样应拒派 GATE_ENTRY_MISSING）
    function checkEntryGate(sessionId, batchId, batch, lane) {
        const t = taskOf(batch, lane);
        if (!t || (t.layer !== 'exec' && t.layer !== 'audit') || !Array.isArray(t.consume) || t.consume.length === 0)
            return { ok: true };
    ...
    function checkPlanContract(sessionId, batchId, batch, lane) {
        const t = taskOf(batch, lane);
        if (!t || t.layer !== 'plan' || !Array.isArray(t.produce))
            return { ok: true };
        const problems = [];
    ...
    // Exit Gate：exec→outputs 存在；audit→produce 存在；plan→Plan 契约
    function checkExitGate(sessionId, batchId, batch, lane) {
        ...
        if (!field || !Array.isArray(t[field]) || t[field].length === 0)
            return { ok: true };
    ...
    function checkNeedHumanGate(sessionId, batchId, batch, lane, note) {
        ...
        if (evidence)
            return { ok: true, declared: true, path: det.path, evidence };
        return {
    ...
    function checkCommandGate(sessionId, batchId, batch, lane, deps = {}) {
        ...
        const t = taskOf(batch, lane);
        if (!t || t.layer !== 'exec')
            return { ok: true, declared: false }; // 仅约束 exec 层验证声明
        ...
        if (!det.declared)
            return { ok: true, declared: false }; // 未声明 gate → 零感知
    ...
    function checkCompleteGate(batch) {
        ...
        if (layers.audit.some((id) => ['failed', 'conflict'].includes(laneState(id))))
            return { ok: false, code: 'GATE_COMPLETE_AUDIT_FAILED' };
    ...
    function gateStatus(sessionId, batchId, lane) {
        ...
        const t = taskOf(batch, lane);
        if (!t)
            return { lane, layer: null, state: batch.lanes[lane], gates: 'generic', team: batch.team };
