...
// lib/comms/topic-runtime.js —— topic 运行时装配（index.js 装配点，生命周期与 trajectory 桥同形）
// 设计要点：
//   - 开关：capabilities.topic.enabled（readCapability 缺省合并 {enabled:false}，默认关）；enabled 时装配面创建
//     运行时（start/stop 供热更新实时启停），关闭时零挂载零路径（与 acps/bridge config 短路同构）
//   - 发布方（首批枚举）：store.setMember/setPhase 调用点埋点（onStateChange → publishStateChange）
...
//     publishConfigChanged（可选镜像）无 session 归属 → 仅进程内分发，不落 mailbox
...
export function createTopicRuntime(ctx, { root, logger } = {}) {
  ...
  // 配置变更镜像（可选）：swarm.config.changed——无 session 归属 → 仅进程内分发
