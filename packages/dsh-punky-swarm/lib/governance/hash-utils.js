...
// governance/hash-utils.js —— 证据信封哈希工具（简版）
// 零依赖（node:crypto 为 Node 标准库内建模块，非 package.json 依赖）。
...
// 完整 RFC 8785（数字规范化 / 逐字符转义表）不做——简版覆盖确定性证据信封需求。
...
// 收据锚定哈希：hash = sha256(canonical(收据除 anchor 外全部字段 + prevHash))——
...
// anchor 对象构造（version 1：sha256 内容哈希链，简版信封）
