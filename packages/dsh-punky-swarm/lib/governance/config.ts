...
// 治理配置默认值与解析：GOVERNANCE_DEFAULTS + resolveGovernanceConfig。
// 默认值：governance.hook.enabled=true 默认开启、可显式关闭。
// resolve 扩 escalation 段（扩 resolve 单点：remount JSON 比较器零改动——
...
// 对齐 resolveWatchConfig 模式（lib/schema.ts）：宽松输入接口（字段 unknown），
