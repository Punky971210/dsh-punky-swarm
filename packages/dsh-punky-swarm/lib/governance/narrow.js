...
function locate(obj, tokens) {
    let cur = obj;
    for (let i = 0; i < tokens.length; i++) {
        if (cur === null || cur === undefined || typeof cur !== 'object')
            return null;
        const container = cur;
        const key = Array.isArray(container) && /^\d+$/.test(tokens[i])
            ? Number(tokens[i])
            : tokens[i];
        if (!Object.prototype.hasOwnProperty.call(container, key))
            return null;
        if (i === tokens.length - 1)
            return { parent: container, key, value: container[key] };
        cur = container[key];
    }
...
export function computeNarrowedParams(params, bounds) {
    const narrowed = deepClone(params);
    const clamped = [];
    let changed = false;
