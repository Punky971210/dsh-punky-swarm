...
// 违规分类器（G3）：六分路 P0-P6 + 兜底，判定顺序 P0→P1→P2→P3→P4→P5→P6→兜底，
// 任一档命中即停（取最高优先级）。fail-closed 纪律：检出违规但无法归类（unknown）→ 兜底原语，绝不 ALLOW。
// 蓝图：m2-detailed.md §4（hf.md:79-87 逐档映射）。纯函数、零依赖、确定性（无 IO/时间/随机）。
// P0 硬化（harden-plan §5.1 B）：兜底从硬编码 DENY 改为读取 config.defaults.deny（kernel 经 ClassifyInput.defaults
//   传入，死配置修复）；「兜底不可 ALLOW」双保险——defaults.deny 若为 ALLOW 回退 DENY（config resolve 已先校验，
//   classify 侧防御兜底，fail-closed 纪律 hook-eval A.4）。
...
export interface ClassifyInput {
  ...
  defaults?: GovernanceConfig['defaults']; // P0：fail-closed 兜底配置（kernel 传入 config.defaults；缺省=DENY）
}
