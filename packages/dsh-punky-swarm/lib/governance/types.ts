...
// 工具调用级护栏（governance hook）纯函数内核共享类型。
// 签名级逐字段对齐。形态决策：TS 纯数据、零依赖（仅类型层，编译期擦除）；消费方 = lib/governance/ 其余组件 + wiring.js。
// Rule 增加显式 narrow?: NarrowBounds[]（对齐 narrow.ts 契约，
...
//   RefusalReceipt 扩展可选 deferMeta?/pauseMeta?/ask? 字段（收据扩展向后兼容：旧收据无字段不炸）。
// 收据签名/哈希锚定——RefusalReceipt 扩展可选 anchor? 字段
//   （sha256 哈希链；同 session 收据按 ts 序串链，篡改任一收据破坏后续链）。
...
// REQUIRE_APPROVAL ask 记录（宿主 serviceAsk 通道接线；dsh 侧不实现审批服务）：
//   pre 同步落盘 initiated（必做）；post 观察者尽力补记 outcome（写失败仅 warn，不改变返回语义）。
...
// 6 治理原语（CAGE GovernanceDecision 六原语）：
//   ALLOW=放行透传 / DENY=拒绝执行 / REQUIRE_APPROVAL=人工审批通道 / DEFER=延后 /
//   NARROW=参数收窄指引 / PAUSE=暂停（简版：DENY/DEFER/NARROW/PAUSE 统一 deny + 收据元信息）。
export type GovernancePrimitive =
  | 'ALLOW' | 'DENY' | 'REQUIRE_APPROVAL' | 'DEFER' | 'NARROW' | 'PAUSE';

// 违规计数升级可计入原语子集：DENY/NARROW 默认计入；DEFER/PAUSE 自带
//   30s/60s 短窗自愈（state-store.js）默认不计、可显式扩入；REQUIRE_APPROVAL（ask 流程）与
//   状态门收据（ruleRefs=[]）设计红线不可配入——resolve 校验层拒绝/回退（见 config.ts）。
export type EscalationPrimitive = 'DENY' | 'NARROW' | 'DEFER' | 'PAUSE';

// 违规计数升级配置（装配键 governance.hook.escalation）——出厂默认关（enabled:false =
...
// 违规类别（classify 六分路 P0-P6 + 兜底 的判档依据）。
export type ViolationCategory =
  | 'hard' | 'pausable' | 'narrowable' | 'soft' | 'manual_review' | 'ftra' | 'unknown';

export interface Violation {
  code: string;            // 规则编号（可溯源；规则编号红线：无来源编号禁止）
...
export interface Rule {
  ...
  narrow?: NarrowBounds[];                 // 显式钳制界限（narrow.ts NarrowBounds 契约）；
                                           //   旧规则无该字段 → 不钳制（向后兼容）
}
...
// 哈希锚定信封（简版）：version=1 表示 sha256 内容哈希链布局。
...
export interface RefusalReceipt {          // 对齐 CAGE RefusalReceipt 简版（字段口径同 evidence 收据）
  ...
  deferMeta?: DeferMeta;                   // DEFER 触发（含状态门 deny）元信息；可选，旧收据无此字段不炸
  pauseMeta?: PauseMeta;                   // PAUSE 触发（含状态门 deny）元信息；可选，旧收据无此字段不炸
  ask?: ReceiptAskMeta;                    // REQUIRE_APPROVAL ask 记录（initiated + 尽力补记 outcome）；可选，旧收据无此字段不炸
  anchor?: ReceiptAnchor;                  // sha256 哈希链锚定（落盘层写/验）；可选，旧收据无此字段不炸
...
export interface KernelDecision {
  ...
  narrowedParams?: NarrowResult;           // 接线后：NARROW / DENY-含-narrowable 时填充（computeNarrowedParams 结果）
  ruleRefs: string[];
}
