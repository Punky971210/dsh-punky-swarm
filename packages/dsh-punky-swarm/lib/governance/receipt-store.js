...
//   verifyRefusals 验链/篡改定位；patchRefusalAsk 事后改写与链交互 = 级联重锚（改收据 → 后续链收据
//   prevHash/hash 级联重算 + ledger 演化快照）。canonical 序列化见 hash-utils.js（RFC8785 简版，零依赖）。
//   WORM 不做；真签名/完整 RFC8785 留待后续（注释留痕）。
...
// 双层桥接事件流（批级可观测，零依赖）：<root>/governance/events/refusal-<sessionId>.jsonl
//   （独立于 refusals 收据目录的批级事件流；只做事件可见性，不触发批级状态迁移）。
...
//   （保持「initiated === ts 同一时刻」契约）。返回守卫后 receipt（原地改 ts/ask.initiated）。
...
// 写入前计算 anchor（prevHash = 链尾锚定收据 hash，首收据 null；hash = sha256(canonical(body+prevHash))），
//   anchor 字段随 json 与 ledger 同行。返回落盘文件绝对路径。
...
export function writeRefusal(root, receipt) {
  ...
  // 锚定：prevHash = 现链尾（本收据尚不在盘上）；hash 覆盖除 anchor 自身外全部字段（含 prevHash）
  ...
  try {
    // 写后读校验（fail closed）：可解析 + receiptId 一致 + anchor 重算一致
...
//   alg 非 sha256 / version 非 1 的旧锚定方案 → 上抛（仅认 sha256 简版链）。
...
// limit 给定时返回最近 N 条（slice(-limit)）。保持兼容：不校验 anchor（读回不炸）。
...
//   旧收据（无 anchor）不参与链校验、不判失败（锚定方案前收据兼容）。
...
export function patchRefusalAsk(root, sessionId, receiptId, outcome) {
  ...
  if (prevAnchor && prevAnchor.alg === 'sha256' && prevAnchor.version === 1) {
    // 级联重锚。本收据重算 hash（prevHash 不变）；其后继 prevHash/hash 逐跳重算至链尾。
