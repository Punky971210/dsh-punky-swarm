...
function checkConditionPaths(t, cond) {
    for (const c of cond ?? []) {
        ...
        if (p.startsWith('artifacts/'))
            throw new Error('task ' + t.id + ' condition cross-batch reference is disabled in MVP (N6): ' + p);
    ...
    const checkPaths = (t, field) => {
        for (const p of t[field] ?? []) {
            ...
            if (isAbsPath(p))
                continue;
            if (p.startsWith('artifacts/'))
                throw new Error('task ' + t.id + ' ' + field + ' cross-batch reference is disabled in MVP (N6): ' + p);
            if (!/^(plan|exec|audit)\//.test(p))
