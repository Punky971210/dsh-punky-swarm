...
test('T0 默认关：enabled=false（缺省）时 worktree 工具不注册，工具总数 14', () => {
  ...
  assert.equal(tools.some((t) => t.name === 'lane_worktree_create' || t.name === 'lane_worktree_merge' || t.name === 'lane_checkpoint' || t.name === 'lane_checkpoint_status'), false);
});
