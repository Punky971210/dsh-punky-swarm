...
- 指挥循环（每 turn）：batch_status 读黑板 → mailbox_send inbox/broadcast 建议派发（**建议派发时从 batch_status 读取 sessionId/产物根注入任务包，禁止手写**） → mailbox_read outbox 收 worker 完成通知 → member_status running→review → member_settle 结算裁决（可用 gate_status 复核 GATE_EXIT_MISSING）
